// components/reports/DefectTrend.tsx
...
export default function DefectTrend() {
  ...
  // 현재는 월 기준 더미 (day/year는 UI만 동작)
  const data = Array.from({ length: 12 }).map((_, i) => {
    const month = i + 1
    const filtered = defectData.filter(
      (d) => new Date(d.occurredAt).getMonth() + 1 === month
    )

    return {
      label: `${month}월`,
      심각: filtered.filter((d) => d.riskLevel === "심각").length,
      주의: filtered.filter((d) => d.riskLevel === "주의").length,
      보통: filtered.filter((d) => d.riskLevel === "보통").length,
    }
  })
